...
def calc_overlap_distance(core_region_left, core_region_right, cur_region_left, cur_region_right, min_overlap=-10):
    ...
    if overlap_left < min_overlap:
        ...
        new_core_left = [ min(core_region_left[0], cur_region_left[0]), max(core_region_left[-1], cur_region_left[-1]) ]
        new_core_right = [ min(core_region_right[0], cur_region_right[0]), max(core_region_right[-1], cur_region_right[-1]) ]
...
#chr1	17938	17987	chr1	17938	17993	1_1	0	+	+	Sample1_ChimericWhole_32558020	1_1
...
for line in open(sys.argv[1]):
    ...
    orignal_clusters[s[-1]].append(s[:-2])
    orignal_clusters[s[-1]][-1].append( s[-2].split(',') )
#print(len(orignal_clusters))
